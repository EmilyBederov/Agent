...
class Display:
    ...
    def parse_recipe_dict(self, recipe_dict):
        """
        ...
        if total_time and isinstance(total_time, str):
            time_str = self.parse_iso_duration(total_time)
        elif prep_time and isinstance(prep_time, str):
            time_str = self.parse_iso_duration(prep_time)
        else:
        ...
        # Parse ingredients
        ingredients = {}
        quantities = self.safe_eval_list(recipe_dict.get('RecipeIngredientQuantities', '[]'))
        parts = self.safe_eval_list(recipe_dict.get('RecipeIngredientParts', '[]'))
        ...
        # Parse instructions
        instructions_list = self.safe_eval_list(recipe_dict.get('RecipeInstructions', '[]'))
        transformed['instructions'] = "\n".join(instructions_list)

        # Parse keywords
        transformed['keywords'] = self.safe_eval_list(recipe_dict.get('Keywords', '[]'))
        ...
        if images == 'character(0)':
            ...
            transformed['images'] = self.safe_eval_list(images)
    ...
    def display_recipes_ranked(self, recipes_list, relevance_scores=None):
        """
        ...
        transformed_recipes = [self.parse_recipe_dict(recipe) for recipe in recipes_list]
        ...
        for i, recipe in enumerate(sorted_recipes):
            self.display_single_recipe(recipe, i+1, len(sorted_recipes))

    def display_single_recipe(self, recipe_dict, rank=None, total=None):
        """
        ...
        if 'RecipeId' in recipe_dict:
            recipe_dict = self.parse_recipe_dict(recipe_dict)
